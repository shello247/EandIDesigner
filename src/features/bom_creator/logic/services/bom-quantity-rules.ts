...
export function calculateProjectedBomQuantity(
  quantityRule: BomQuantityRule,
  baseQuantity: number,
  facts: BomAssemblyQuantityFacts
): BomQuantityResult {
  if (quantityRule === "manual") {
    return { status: "manual_required" };
  }
  ...
  if (quantityRule === "per_cable_end") {
    return {
      quantity: baseQuantity * facts.cableEndCount,
      status: "calculated"
    };
  }

  if (quantityRule === "per_conductor_termination") {
    return {
      quantity: baseQuantity * facts.conductorTerminationCount,
      status: "calculated"
    };
  }

  if (quantityRule === "per_connection") {
    return {
      quantity: baseQuantity * facts.connectionCount,
      status: "calculated"
    };
  }

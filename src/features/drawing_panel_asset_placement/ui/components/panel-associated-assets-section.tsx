...
export function PanelAssociatedAssetsSection({
  panelLabel,
  targetKind = "physical_backplane",
  items,
  onPlaceAsset
}: {
  panelLabel: string;
  targetKind?: "physical_backplane" | "connection_reference";
  ...
  const targetLabel =
    targetKind === "connection_reference" ? "panel view" : "backplane";
      ...
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="text-sm font-bold">
          <button
            ...
            aria-label={`${isExpanded ? "Collapse" : "Expand"} Associated Panel Assets`}
          ...
          >
            ...
            <span>Associated Panel Assets</span>
          </button>
        </h2>
        <p className="mt-1 truncate pl-[23px] text-xs text-slate-500">
          {panelLabel}
        </p>
      </div>
      {isExpanded ? (
        <div
          id={contentId}
          className="max-h-[360px] space-y-2 overflow-auto p-4"
        >
          {visibleItems.length === 0 ? (
            <div className="flex items-start gap-2 rounded-md border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-800">
              ...
              />
              <span>All associated assets are placed in this {targetLabel}.</span>
            </div>
          ) : null}
          ...
          {placedCount > 0 ? (
            <p className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500">
              {placedCount} associated{" "}
              {placedCount === 1 ? "asset is" : "assets are"} already placed in
              this {targetLabel}.
            </p>
          ) : null}
        </div>

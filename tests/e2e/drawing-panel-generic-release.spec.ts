  ...
  try {
    await test.step("load the generic panel context", async () => {
      ...
      await page
        ...
        .getByRole("row", { name: /MCP-201 Detailed Panel Drawing Detailed Panel/ })
        .getByRole("button", { name: "Load" })
        .click();
      const panelSummary = page.getByRole("complementary", {
        name: "Symbol library"
      });
      await expect(panelSummary.getByText("MCP-201", { exact: true })).toBeVisible();
      await expect(
    ...
    await test.step("discover and place all differently sized terminal strips", async () => {
      ...
      await queue.getByRole("tab", { name: /Terminal Map/ }).click();
